...
class ReportM
{
  ...
  public function getStockCount2($searchVal = null)
  {
    if ($searchVal) {
      $query = "SELECT *, CASE WHEN TEMP.prodt_qty > TEMP.prodt_rord THEN 'OK' ELSE 'NOT OK' END AS prodt_stat FROM 
      (SELECT P.prod_code AS prodt_id, P.prod_name AS prodt_name, P.prod_reod_level AS prodt_rord, SUM(S.stok_quantity) AS prodt_qty, COUNT(S.stok_loca_code) AS prodt_locs FROM tabl_product P LEFT JOIN tabl_stock S ON P.prod_code = S.stok_prod_code WHERE P.prod_active = 1 GROUP BY P.prod_code) AS TEMP 
  ...
  public function getStockData2($data)
  {
    // var_dump($data);
    $sortType = "ASC";
    if ($data['sort_type'] == '1') {
    ...
    if ($data['search_val']) {
      ...
      $query = "SELECT *, CASE WHEN TEMP.prodt_qty > TEMP.prodt_rord THEN 'AVAILABLE' ELSE 'LOW' END AS prodt_stat 
      FROM (SELECT P.prod_code AS prodt_id, P.prod_name AS prodt_name, P.prod_reod_level AS prodt_rord, SUM(S.stok_quantity) AS prodt_qty, 
      COUNT(S.stok_loca_code) AS prodt_locs FROM tabl_product P LEFT JOIN tabl_stock S ON P.prod_code = S.stok_prod_code WHERE P.prod_active = 1 GROUP BY P.prod_code) AS TEMP 
      WHERE TEMP.prodt_qty < TEMP.prodt_rord ORDER BY :ordcol $sortType LIMIT :offset, :rowcon";
      $param = [
        'ordcol' => $data['sort_col'],
        'rowcon' => $data['max_rows'],
        'offset' => $data['row_offset'],
