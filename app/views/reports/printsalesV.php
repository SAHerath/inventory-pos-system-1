...
$pdf->MultiCell(35, '', 'Category', $border, 'L', 0, 1, $x + 110, $y, true);
$pdf->MultiCell(20, '', 'Quantity', $border, 'L', 0, 1, $x + 145, $y, true);
$pdf->MultiCell(25, '', 'Date', $border, 'L', 0, 1, $x + 165, $y, true);
...
foreach ($data['sales'] as $item) {
  $categLen = ceil(strlen($item['sales_catg']) / 18);
  $descpLen = ceil(strlen($item['sales_desp']) / 50);
  $cellH = max($categLen, $descpLen) * 6;

  $pdf->MultiCell(25, $cellH, 'PR-' . str_pad($item['sales_pid'], 8, '0', STR_PAD_LEFT), $border, 'L', $fill, 1, $x, $y, true, 0, false, true, $cellH, 'M', false);

  $pdf->MultiCell(85, $cellH, str_replace(PHP_EOL, ' ', $item['sales_desp']), $border, 'L', $fill, 1, $x + 25, $y, true, 0, false, true, $cellH, 'M', false);

  $pdf->MultiCell(35, $cellH, $item['sales_catg'], $border, 'L', $fill, 1, $x + 110, $y, true, 0, false, true, $cellH, 'M', false);

  $pdf->MultiCell(20, $cellH, $item['sales_qty'], $border, 'L', $fill, 1, $x + 145, $y, true, 0, false, true, $cellH, 'M', false);

  $pdf->MultiCell(25, $cellH, $item['sales_date'], $border, 'L', $fill, 1, $x + 165, $y, true, 0, false, true, $cellH, 'M', false);
